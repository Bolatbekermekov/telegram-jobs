...
# this job» стоит на всех 242, а перебор ссылок ниже дал верный адрес только на
# 172. Шестьдесят раз он промолчал (внешних хостов на странице оказывалось
# несколько) и четыре раза увёл не туда. Худший из четырёх —
# `remocate.app/jobs/director-of-reward-and-people-analytics`: перебор вернул
...
# писалось бы по блогу. Со структурной кнопкой те же 242 карточки разбираются
# без единого промаха: 235 адресов и 7 отказов там, где адреса нет (см. ниже).
#
# Кнопка узнаётся по ДВУМ признакам сразу, и ни одного по отдельности не хватает:
#
#  * класс `button` — это Webflow, у него так подписаны все кнопки сайта, в том
#    числе «Blog» и «Post a job» в шапке;
#  * надпись «apply» — она встречается и в описании: на карточке
#    `middle-fullstack-developer-next-js-hono-drizzle-ai-sdk` автор поставил в
#    текст ссылку «Apply here» на Google Forms, и это не кнопка.
#
# Вместе они на всех 242 карточках дали ровно одного кандидата.
_ANCHOR_RE = re.compile(r"(?is)<a\b([^>]*)>(.*?)</a>")
_HREF_ATTR_RE = re.compile(r'(?is)\bhref\s*=\s*"([^"]*)"')
_BUTTON_CLASS_RE = re.compile(r'(?is)\bclass\s*=\s*"[^"]*\bbutton\b')
# Начало надписи, а не вхождение: «Apply» в «Applying is easy» — не кнопка.
_APPLY_LABEL_RE = re.compile(
    r"(?i)^(?:apply\b|откликн|подать заявку|отправить отклик)")


def _apply_button_href(html_text) -> str | None:
    """href кнопки отклика, или None, если кнопки на странице нет вовсе.

    Пустая строка — это ОТВЕТ, а не отсутствие ответа: `href="#"` Webflow
    рисует у той же кнопки, когда у карточки нет страницы отклика и откликаться
    надо письмом (рядом лежит заполненный `contact-wrapper` с почтой и
    телеграмом). Таких карточек 7 из 242, и на одной из них перебор ссылок
    вернул `88publishing.com` — главную страницу компании, где отклика нет
    вовсе. Кнопка без адреса значит «страницы отклика нет», а не «угадывай
    дальше».
    """
    found = None
    for attrs, inner in _ANCHOR_RE.findall(str(html_text or "")):
        if not _BUTTON_CLASS_RE.search(attrs):
            continue
        label = " ".join(_html.unescape(_ANY_TAG_RE.sub(" ", inner)).split())
        if not _APPLY_LABEL_RE.match(label):
            continue
        m = _HREF_ATTR_RE.search(attrs)
        href = _html.unescape(m.group(1)).strip() if m else ""
        if found is not None and found != href:
            return ""      # две РАЗНЫЕ кнопки — выбирать между ними нельзя
        found = href
    return found


def aggregator_apply_url(html_text, page_url: str) -> str:
    """Настоящий адрес отклика со страницы агрегатора, или "" если его не видно.

    Сначала кнопка: её поставил сам агрегатор, и спорить с ней нам нечем.
    Перебор внешних ссылок остался запасным вариантом — для страницы, где
    кнопки нет (другая вёрстка, другой агрегатор), — и там по-прежнему молчит,
    когда кандидатов больше одного. Тот же принцип, что у точки входа Easy
    Apply, где угадывание между похожими контролами уводило прогон в чужую
    вакансию: пусто означает «пусть человек посмотрит сам», и это честнее, чем
    отклик не туда.

    `_NOISE_HOSTS` к кнопке НЕ применяется, и это не недосмотр. Список отвечает
    на вопрос «может ли этот хост быть работодателем», который стоит только
    перед перебором; кнопка на него уже ответила. Замер: из 242 карточек пять
    ведут кнопкой на «шумный» хост, и для четырёх это настоящий адрес отклика —
    Google Docs с описанием вакансии (3 карточки) и объявление на LinkedIn.
    """
    brand = _brand_of(page_url)
    button = _apply_button_href(html_text)
    if button is not None:
        # Только absolute http(s): `#` — кнопка без страницы отклика, а
        # `mailto:`/`tg://` — контакт, на который `page.goto` не ходит.
        if not _SCHEME_RE.match(button):
            return ""
        host = _urlparse(button).netloc.lower()
        return "" if brand and brand in host else button
    by_host: dict = {}
