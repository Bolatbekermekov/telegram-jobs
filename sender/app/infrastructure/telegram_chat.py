"""Человек это или канал — спрашиваем у самого Telegram (близнец интейковой).
...
# повторяет ник подписи дважды. Прогон живёт минутами и читает десятки лидов,
# так что кэш переживает весь прогон — это ровно та память, которая тут нужна.
