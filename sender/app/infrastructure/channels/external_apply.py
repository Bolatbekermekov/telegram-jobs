...
_SCRAPE_JS = r"""() => {
  ...
  // Подпись это то, что ОСТАЁТСЯ от <label>, когда убрать сам виджет. Ветку, в
  // которой лежит контрол, целиком выбрасываем: там живут внутренности виджета,
  // а не подпись поля.
  //
  // Живьём 2026-09-16 (лид #1301) и снова 2026-09-17 (лид #1392, Workable
  // Action1) подпись телефона выходила «*Phone+7United States+1United
  // Kingdom+44Canada…». По ней правило не узнавало телефон, вопрос уезжал
  // модели, та честно писала номер владельца — и страж личных данных снимал уже
  // заполненную форму в ручные.
  //
  // Первая попытка (fbdd9aa) вырезала `select` и `option` и не лечила ничего:
  // проверка на живой странице показала, что списка стран там нет вовсе. Это
  // intl-tel-input — коды нарисованы как `<li role=option>` внутри соседнего
  // <div>, а рядом с ним <span>*Phone</span>, и это и есть подпись. Тот тест
  // проверял выдуманную разметку и потому проходил впустую.
  //
  // Правило по ветке с контролом чинит заодно и согласие на той же форме: его
  // подпись читалась как «SVGs not supported by this browser.» (запасной текст
  // картинки), а стала настоящим текстом про Privacy Notice. Текст, лежащий
  // РЯДОМ с контролом, а не внутри отдельной ветки, не страдает — так устроены
  // одинокие галочки и варианты радиогрупп.
  const labelText = l => {
    const c = l.cloneNode(true);
    const CTRL = 'input,select,textarea,button';
    [...c.children].forEach(ch => {
      const tag = ch.tagName.toLowerCase();
      if (['input', 'select', 'textarea', 'button'].includes(tag)
          || ch.querySelector(CTRL)) ch.remove();
    });
...
#
# Попутно там же нашлось, что `known_ats_iframe` сравнивал подстроку со ВСЕМ
# адресом iframe, а не с его хостом: на той же странице DataDome отдаёт
# `geo.captcha-delivery.com/captcha/?…&referer=https%3A%2F%2Fjobs.smartrecruiters
# .com%2F…`, и «smartrecruiters.com» из ПАРАМЕТРА засчитывалось за вендора —
# первый же успешный клик увёл бы браузер в капчу. Это ПОЧИНЕНО: сравнение
# переехало на `host_allowed`, по границам меток (см. `classify_apply`). Чат-бот
...
def _verify_submitted(page, url: str, submit_before: int = -1) -> None:
    ...
    if said:
        # Разметку сюда тоже. Живьём 2026-09-12, лид #1140 (BambooHR): «форма не
        # приняла: State *» — и разобрать нечем, потому что эта ветка, в отличие
        # от соседней, страницу не сохраняла. Поле «State» может быть и списком
        # штатов, и текстом, и обязательным лишь для США; какое оно — видно
