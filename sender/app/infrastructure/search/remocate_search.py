...
ВТОРАЯ: глубину надо ограничивать. Доля мёртвых по страницам ленты: 1-я — 15%,
3-я — 25%, 10-я — 75%, 30-я — 60%, 70-я — 70%. Обрыв лежит между третьей и
десятой, поэтому живой запас — примерно первые три страницы, около 60 вакансий
(REMOCATE_PAGES). Remocate — это не «две тысячи вакансий» (в разделе их 102
страницы по 20), а быстрая узкая лента, которую надо опрашивать часто и
неглубоко.
...
# Три страницы — граница живого запаса из замера выше, см. app/config.py.
DEFAULT_PAGES = 3
...
class RemocateSearcher:
    ...
    def __init__(self, feed_url: str = REMOCATE_FEED_URL,
        ...
        # правило было бы вредным: в ленте 102 страницы, а живого — первые три,
