...
_QUESTIONS_SYSTEM = (
    ...
    "смыслу. Без пояснений вне JSON. "
    "ЗАРПЛАТНЫЕ ОЖИДАНИЯ считай так: уровень — MIDDLE (не junior и не senior), "
    "даже если вакансия названа иначе; рынок — СТРАНА вакансии, а не своя; для "
    "стран Европы бери СРЕДНЮЮ по этой стране вилку для такой роли и уровня, а не "
    "верх вилки. НЕ ЗАВЫШАЙ: завышенная цифра снимает кандидата с рассмотрения "
    "молча. Указывай число в ВАЛЮТЕ рынка вакансии и в том периоде, о котором "
    "спрашивают (в месяц или в год); если период не назван — в год для Европы и "
    "США, в месяц для СНГ. Если поле принимает только цифры, верни одно число."
)
...
_SYSTEM = (
    ...
    "ФОРМАТ: строка 'Мой стек:' с технологиями через запятую, не более 7. "
    "Отдельным предложением 'Плюс к пониманию системы целиком:', не более 4. "
    "Если в вакансии названы инструменты (Postman, "
