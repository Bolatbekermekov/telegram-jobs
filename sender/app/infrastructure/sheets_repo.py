...
#
# `manual` сюда НЕ входит, и это решение, а не забывчивость. Статус пишется из
# семи мест, и в шести до человека не дошло ничего: пустой «Источник», кривой
# хендл Threads, придержанный до глаз контакт из Threads, отсутствующая сессия
# Threads, снятая вакансия и форма отклика, которую не удалось заполнить.
# Внести статус целиком значит молча похоронить живого лида — рекрутёр, чью
# вакансию сняли, перестал бы получать письма и по следующей, ни разу ничего
# от нас не получив.
#
# Седьмое место — отклонённое, отозванное или истёкшее приглашение — до
# человека действительно дошло. Но README закрепляет решение, слать ли запрос
# повторно тому, кто один раз отказал, за владельцем аккаунта; автоматический
# пропуск это решение отбирает, и притом терминально: `skipped` из очереди не
# возвращается, и такой строки владелец больше не увидит.
