...
# Свои слова, а не общий SEARCH_KEYWORDS. Решение владельца 2026-09-12: выдача
# Indeed привязана к США, и по общим словам возвращается американская удалёнка,
...
# Свой потолок на запрос у LinkedIn — потому что там запрос это «слово × страна»,
# а не просто слово. Арифметика, из-за которой роли не искались:
#   бюджет площадки 250 ÷ 25 карточек за запрос = 10 запросов за прогон,
#   а пар «слово × страна» — 14 × 12 = 168.
# То есть за прогон опрашивалось 6% матрицы. Ротация (rotate_by) спасала от
# «вечно одних и тех же первых пар», но не от редкости: конкретная пара
# «qa engineer × United Arab Emirates» выпадала примерно раз в семнадцать
# прогонов. Отсюда и ощущение, что мобилки и QA почти нет, — их искали редко,
# а не «их мало».
# 10 вместо 25 переворачивает счёт: 250 ÷ 10 = 25 запросов, и все 14 слов
# успевают пройти КАЖДЫЙ прогон. Берём при этом верхние 10 из выдачи, а LinkedIn
# сортирует по релевантности — то есть теряем хвост страницы, а не лучшее.
# Это НЕ старая ошибка «бюджет делится между словами» (она давала по одной
# карточке на запрос, см. per_keyword_limit): здесь потолок фиксированный.
# hh это не касается — у него свой SEARCH_PER_KEYWORD.
LINKEDIN_PER_KEYWORD = int(os.environ.get("LINKEDIN_PER_KEYWORD", "10"))
# И свой у Wellfound — по той же арифметике и с тем же исходом. Замер
# 2026-09-02: бюджет площадки 250 ÷ 25 карточек за запрос = 10 запросов, а
# слагов ролей у нас 13. Обход идёт «страница снаружи, слово внутри», значит
# бюджет кончался на первой странице десятого слага, и три роли — мобилка, Vue,
# node — не опрашивались НИКОГДА. Настройка WELLFOUND_PAGES при этом ни на что
# не влияла: до второй страницы дело просто не доходило.
#
# Лента при этом глубокая и не вырождается: шесть страниц подряд по трём ролям
# дали 247, 180 и 177 РАЗНЫХ вакансий, причём на шестой странице новыми были
# 43 из 43, 25 из 25 и 28 из 28. Упирались мы в свой потолок, а не в источник.
WELLFOUND_PER_KEYWORD = int(os.environ.get("WELLFOUND_PER_KEYWORD", "10"))
...
#              HTTP, а не браузером. 150 = ~6 мин вместо полутора часов.
#   LinkedIn — было ~19 с, стало ~2,5 с тем же способом. 150 = ~6 мин вместо
#              сорока семи, и полторы сотни заходов под логином на страницы
#              вакансий больше не делаются вовсе (HTTP идёт анонимно).
...
# Отсюда: потолок 150 больше НЕ упирается во время ни на одной из двух площадок,
# он упирается только в MATCH_MAX_JOBS выше — поднимать их имеет смысл вместе,
# поодиночке ни один из них ничего не добавит. Замер прогона 2026-08-27: оценили
# 150, набрали 29 при MATCH_MAX_JOBS=30 — уперлись в оба потолка сразу.
