...
def _refetch(url: str) -> str:
    """Перечитать вакансию: дешёвым HTTP, браузером — только там, где иначе никак.

    Браузер стоит ~38 с против 0.6 с у HTTP, поэтому решение «звать ли его»
    принимает refetch_vacancy, и зовёт только для hh: у него простой GET
    получает 403 от антибота, и без этого хода любая присланная боту hh-ссылка
    вставала намертво (лид #991, 2026-09-11).
    """
    from app.application.refetch_vacancy import refetch_vacancy
    from app.infrastructure.channels.headhunter import hh_vacancy_text

    return refetch_vacancy(
        url,
        http_read=lambda u: fetch_vacancy_text(u, timeout=_REFETCH_TIMEOUT),
        browser_read=lambda u: hh_vacancy_text(u, config.HH_STATE_PATH,
