...
ALLOWED_APPLY_HOSTS = frozenset({
    ...
    "factorialhr.com",
    # Замер 2026-09-17 (лид #1409): careerplug.com — вендор, и доказано это тем
    # же, чем у Rippling ниже, только с обратным исходом. Поддомен арендатора
    # лежит в ЗОНЕ вендора: eitacies-inc.careerplug.com и служебный
    # app.careerplug.com идут одним CNAME на a7e4bea27a309a595
    # .awsglobalaccelerator.com, NS зоны careerplug.com — AWS. Арендаторов
    # тысячи (второй проверен живьём: cplugjobs.careerplug.com).
    # А главное — техника не мешает. Форма движковая: на двух вакансиях Eitacies
    # (#3498048, #3593166) один и тот же набор из 14 полей, подписи человеческие
    # («First Name*», «Email*», «Phone*», «ZIP Code», «Recent Employer»), имена
    # рельсовые (`app[applicant_attributes][...]`) — их печатает движок, а не
    # работодатель. Ни одного безымянного обязательного поля, логина нет,
    # reCAPTCHA невидимая (как у Ashby).
    # Известная цена, замерена там же: у ОБОИХ файловых входов подпись «Upload
    # File», а настоящие заголовки («Resume/CV *» и «Cover Letter») лежат на
    # 5–7 уровней выше в DOM, куда скрапер не дотягивается. Значит
    # `_only_the_real_resume_field` ничего не отсечёт, и резюме уедет в оба
    # поля: работодатель получит его же вместо сопроводительного письма.
    # Поле необязательное, персональные данные никуда мимо не уходят, вслепую
    # модель ничего не отвечает — на фоне Rippling это опечатка, а не подлог.
    # Чинится не списком, а подписью из окружающего блока (см. конец заметки
    # про Rippling).
    "careerplug.com",
})

# --- проверено и НЕ добавлено 2026-09-17 -------------------------------------
#
# recruiting.paylocity.com (лид #1396). Многоарендность есть: с одного хоста
# раздаются доски Spatial Front, Office Practicum, PuzzleHR, Solerity и других
# — то есть по доводу «вендор» он прошёл бы. Останавливает форма, и ровно тем
# же, чем Rippling (замерено на живой вакансии Spatial Front #3973455): из 28
# полей ДВЕНАДЦАТЬ текстовых пришли без подписи и без
# имени (среди них имя, фамилия, почта, телефон), при этом «Country»,
# «Address Line 1», «City», «County», «State», «Zip Code» обязательные — модель
# отвечала бы вслепую, а ответ уехал бы работодателю адресом проживания.
# Ждали 15 секунд после загрузки: подписи не появляются, это не гонка рендера.
# Файловых входов четыре, три принимают .doc/.docx/.pdf, ни один не подписан —
# `_only_the_real_resume_field` не отсечёт ничего, резюме уедет во все три.
#
# jobs.gusto.com. Форму увидеть не удалось: хост отвечает 403 и страницей
# «Один момент…» — это заслон Cloudflare. Обходить его мы не будем, а разрешать
# хост, формы которого мы ни разу не видели, значит разрешать вслепую.
#
# agilefuel.com, cloudgeometry.com, sourceo.io, workfully.com — сайты ОТДЕЛЬНЫХ
# компаний, то есть ровно то, от чего этот список защищает. У agilefuel форма
# приличная (8 полей на Webflow, «First name *», «Link to your CV *»), но это
# своя вёрстка одной компании: разрешение стоило бы одной вакансии и перестало
# бы работать с первой же переделкой сайта. На странице вакансии cloudgeometry
# формы отклика нет вовсе — скрапер ловит «Enter your work email*» рассылки и
# чат «Ask CloudGeometry AI», отклик там идёт письмом на jobs@cloudgeometry.com.
# sourceo.io/careers отдаёт ноль полей. workfully.com — витрина агентства,
# вакансий для кандидата на ней нет, единственное поле «Email*» это подписка.
...
#
# ПЕРЕЗАМЕР 2026-09-05 — вопрос закрыт, и не по той причине, что обсуждалась.
# Формы из семи полей больше нет. Под «Apply now» теперь ОДНО поле — «Email», —
# кнопка «Next», рядом «Continue with Google», и подпись «By continuing, you
# agree to all terms & privacy policies». То есть это не форма отклика, а
# регистрация: нажатие принимает чужие условия и заводит аккаунт.
#
# Строить многошаговый драйвер стало не под что: заполнять нечего, а шаг, до
# которого он должен был дойти, закрыт учётной записью. Ни завести её, ни
# принять условия автоматика не вправе — это делает владелец, руками, один раз;
# только после этого имеет смысл смотреть, что за аккаунтом.
#
# Из 11 вакансий (16 лидов) на 2026-09-05 живы 8, три отдают «Explore other
# opportunities». Так что цена ожидания — не «16 лидов», а восемь, и она тает.
