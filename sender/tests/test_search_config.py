...
def test_remocate_depth_default_matches_the_measurement():
    """Пять страниц — это граница живого запаса, а не круглое число.

    Sitemap (5049 ссылок) взять нельзя: 65% вакансий там мертвы, до отправки
    дошла бы одна из сорока. Глубину ленты сначала обрезали на третьей странице
    по случайным выборкам («1-я — 15%, 3-я — 25%, 10-я — 75%»), но сплошная
    проверка 2026-08-27 — все 200 карточек первых десяти страниц тем же
    `vacancy_alive`, что стоит в прогоне, — поставила обрыв в другое место: 1-я
    20%, 2-я 35%, 3-я 25%, 4-я 20%, 5-я 20%, 6-я 55%, 7-я 60%, 8-я 40%, 9-я 35%,
    10-я 70%. Четвёртая и пятая живы как первая.

    Второй довод — цена: описание у remocate уже в кэше ленты, оценка стоит
    1,18 с (на hh — ~38 с, там за описанием идёт отдельная страница). По не
    виданным раньше карточкам того же вечера страницы 4-5 дали 2 живых подходящих
    лида за 29 оценок, а страницы 6-10 — 1 лид за 79. Прогон 2026-08-27 с тремя
    страницами дал один лид, с пятью дал бы три.
    """
    assert config.REMOCATE_PAGES == 5
