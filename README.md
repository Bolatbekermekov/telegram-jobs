...
> Отдельно он **не отправляет** лид из Threads, если контакт не однозначен — его
> предложила модель или это просто `@упоминание` в тексте без подсказки на отклик
> («пишите», «резюме», «Telegram:»). Подтверждение и есть проверка такого контакта,
> поэтому без него лид получает статус `manual`, а в заметку идёт причина, кому он был
> адресован и ссылка на тред. В очередь он больше не попадает: чтобы вернуть его,
> поставь Статус обратно в `new`.
