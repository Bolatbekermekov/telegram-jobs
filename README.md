...
| Переменная | Что это |
|---|---|
| `INTAKE_LLM_PROVIDER` / `SENDER_LLM_PROVIDER` | Кто отвечает на вызовы модели: `openai` (по умолчанию), `nvidia` или `gemini`. См. «Смена LLM-провайдера» ниже |
...
| `NVIDIA_API_KEY` | Ключ `nvapi-…` с [build.nvidia.com](https://build.nvidia.com). Нужен, только если провайдер = `nvidia` |
| `NVIDIA_MODEL` / `NVIDIA_MODEL_CHEAP` | Модели NIM, по умолчанию обе `minimaxai/minimax-m3` |
| `GEMINI_API_KEY` | Ключ `AIza…` из [Google AI Studio](https://aistudio.google.com/apikey). Нужен, только если провайдер = `gemini` |
| `GEMINI_MODEL` / `GEMINI_MODEL_CHEAP` | Модели Gemini, по умолчанию обе `gemini-3.5-flash-lite` |
...
| `REMOCATE_PAGES` | Глубина ПЕРВОГО прохода Remocate — по разделу `development` (по 20 вакансий на страницу; в разделе их 102). По умолчанию `5` — дальше лента мёртвая: сплошная проверка всех 200 карточек первых десяти страниц (2026-08-27, тем же `vacancy_alive`, что стоит в прогоне) дала 20% мёртвых на 1-й, 20% на 4-й и 5-й, но уже 55% на 6-й и 70% на 10-й. Глубже пятой сканирование перестаёт окупаться: страницы 4-5 — 29 оценок на 2 живых подходящих лида, страницы 6-10 — 79 оценок на 1 |
| `REMOCATE_HOME_PAGES` | Глубина ВТОРОГО прохода Remocate — по главной `remocate.app` (в ней 273 страницы). По умолчанию `3`. Проход нужен потому, что разделов у сайта 15 и вакансия лежит ровно в одном (попарные пересечения выкачанных целиком разделов равны нулю), а у 102 объявлений категории нет вовсе — их видно ТОЛЬКО на главной; за 90 дней таких 57, среди них Senior Data Engineer, QA Engineer (Data Stack), Java Backend Developer, Senior React Native Developer, DevOps Engineer. Главная — дополнение, а не замена: 5 страниц раздела дают 79 карточек, прошедших предфильтр, 5 страниц главной — 35, и лишь 8 из них новые. `3` = точка, где отдача кончается (4-я страница даёт +0 новых), и всё ещё шесть дней публикаций — с запасом на воркер, который ищет ~3 раза в сутки. Дубли между проходами отбрасываются (30 из 100 карточек главной уже стояли в разделе). Ноль или пусто = вернуться к трём |

---

## Смена LLM-провайдера

Кроме OpenAI проект умеет ходить в **NVIDIA NIM** ([build.nvidia.com](https://build.nvidia.com))
и в **Google Gemini** ([AI Studio](https://aistudio.google.com/apikey)). У обоих есть
OpenAI-совместимый endpoint, поэтому работает тот же SDK: меняются только ключ,
адрес и имена моделей.

Переключатели раздельные, по одному на половину:

```bash
SENDER_LLM_PROVIDER=gemini   # рассылка и поиск на ноуте
INTAKE_LLM_PROVIDER=gemini   # бот-приёмник в облаке
```

Раздельные они не для симметрии. У Vercel лимит функции **10 секунд**, а бот
зовёт модель синхронно из вебхука: медленная модель там означает не «дольше», а
«вакансия не сохранилась». У ноутбука такого потолка нет. Поэтому нормальный
порядок — сначала перевести `sender`, пожить с ним, и только потом трогать бота.

**Что доступно на ключе NVIDIA.** Каталог `/v1/models` показывает 81 модель, но
доступна лишь часть: остальные отвечают `404 Not found for account`. Проверить
свой ключ:
...
**Почему по умолчанию `minimaxai/minimax-m3`.** Замер 2026-09-07 на реальном
промпте бота: русский системный промпт держат только `minimax-m3` и
`moonshotai/kimi-k3`, все Nemotron и `gpt-oss-20b` отвечают по-английски. У
kimi при этом разброс 16–52 с, у minimax — 7.4 с. Язык здесь не косметика:
колонка «Вакансия» идёт прямо в письмо HR.

**Чего ждать.** Пул общий и без гарантий: в замере дважды прилетало
`503 Service temporarily overloaded`, а пять моделей не ответили за 300 секунд.
Остаток кредитов из API не виден — только в кабинете build.nvidia.com.

### Лимиты — то, на чём выбор и ломается

Замер 2026-09-07 на реальных промптах проекта. Считается не скорость, а **сколько
запросов в минуту пропускает ключ**: боту нужно два вызова подряд на каждое
сообщение (саммари + оценка релевантности), а поиску на ноуте — по одному на
каждую найденную вакансию.

| | NVIDIA `minimax-m3` | Gemini `3.5-flash-lite` |
|---|---|---|
| Запросов в минуту | **~2** | **~15** (14 / 18 / 18 в трёх раундах) |
| Саммари вакансии | 1–2 с | 0.7–0.9 с |
| Скоринг | 1.2 с, но второй вызов подряд = 429 | 0.7–1.0 с, оба проходят |
| Язык оригинала | соблюдает | соблюдает |

С NVIDIA бот сохраняет лид, но **молча теряет оценку релевантности**: второй вызов
упирается в 429, а ретраить внутри 10-секундной функции Vercel негде — ключ
отдаёт следующий запрос только через полминуты и `Retry-After` не присылает.
По той же причине на NVIDIA не работает поиск: `run_search.py` изолирует сбои
по площадкам, и первый же 429 обрывает площадку целиком.

Обе модели Gemini — `gemini-3.5-flash-lite`, и к этому пришли не сразу.

Сначала пишущим тиром стоял `gemini-3.5-flash`: `flash-lite` в трёх письмах из
трёх вставлял русскую строку «Мой стек: …» в английское письмо, а `flash` — ни
разу. Причина оказалась не в модели, а в промпте: формат диктует подпись
дословно и в кавычках («строкой `'Мой стек:'`»), а правило языка требует «без
единого русского слова». Послушная модель считала подпись частью ФОРМАТА.
`language_rule` теперь называет английские подписи прямо (`My stack:`), и
`flash-lite` даёт 4 чистых письма из 4.

Дальше решает квота: у `flash` **20 запросов в минуту**, и прогон на нём встаёт
— «Please retry in 52s» через раз. Модели `gemini-2.5-*` новым ключам отвечают
404 — брать 3.x.

Про лимиты вообще: 429 теперь не роняет прогон. `app/infrastructure/rate_limit.py`
читает срок из ответа провайдера (Gemini пишет его в ТЕКСТ ошибки, заголовка
`Retry-After` не присылает) и ждёт столько, сколько названо, с потолком в 120
секунд. Угадывать бесполезно — первая версия ждала по лестнице 20/40/65 и всё
равно мазала мимо окна.

Учти про free tier Gemini: Google использует данные для улучшения продуктов.
Через бота идут чужие тексты вакансий, через sender — твоё резюме.

Если провайдер выставлен, а его ключ пуст, приложение падает на старте с явным
сообщением, а не уходит молча в OpenAI: ключи не взаимозаменяемы — `nvapi-…` в
`api.openai.com` даёт 401.
...
4. В **Environment Variables** добавь: `OPENAI_API_KEY`, `OPENAI_MODEL_CHEAP`, `TELEGRAM_BOT_TOKEN`,
   ...
   `.gitignore`).
   `INTAKE_LLM_PROVIDER` заливать не нужно: без неё бот остаётся на OpenAI.
   Для Gemini добавь `INTAKE_LLM_PROVIDER=gemini` и `GEMINI_API_KEY`, для NVIDIA —
   `INTAKE_LLM_PROVIDER=nvidia` и `NVIDIA_API_KEY`. Перед выбором прочитай
   «Смена LLM-провайдера»: у функции Vercel лимит 10 секунд, а у ключа — свой
   лимит запросов в минуту, и второе важнее.
   Переменные окружения **не применяются к уже собранному деплою** — после правки
   нажми Redeploy.
...
| **Wellfound** | Один раз `make login_wellfound` — и **не закрывай открывшееся окно Chrome**: и поиск, и отклик ходят только через него (Cloudflare привязывает пропуск к прошедшему проверку браузеру, свой не годится). Порт и профиль настраиваются `WELLFOUND_CDP_PORT` / `WELLFOUND_CHROME_PROFILE`. Закрытое окно = ноль вакансий и ноль откликов; прогон, поиск и воркер теперь предупреждают об этом заранее. |
...
шести площадках — LinkedIn, Wellfound, hh, Remocate, Remotive и RemoteOK, — а ты отбираешь их с
телефона. Отдача у них очень разная, и две из шести дают ноль не потому, что сломались:
см. «Сколько какая площадка реально отдаёт» ниже.
...
### Сколько какая площадка реально отдаёт

Замеры 2026-08-27, живьём. Цифры важны: «пусто» в логе у половины площадок — это **их
нормальный рабочий день**, а не сбой поиска, и путать одно с другим дорого.

| Площадка | Отдаёт за прогон | Почему столько |
|---|---|---|
| **hh** | ~29 лидов | Работает. Упирается в потолок оценок, а не в ленту. |
| **Remocate** | 1–3 лида | Источник публикует 2.3 вакансии в сутки — больше взять неоткуда. |
| **LinkedIn** | по-разному | Зависит от квоты приглашений на аккаунте. |
| **Wellfound** | 0 без Chrome | Нужен `make login_wellfound` с **открытым** окном, см. ниже. |
| **Remotive** | 0 новых | Публичная лента — 18–20 вакансий ВСЕГО. Исчерпана, см. ниже. |
| **RemoteOK** | 0 | 96 из 99 вакансий за платной подпиской. |

**Remotive исчерпан — это потолок площадки, а не наша недоработка.** Публичный API
`https://remotive.com/api/remote-jobs` отдаёт 18 вакансий и сам это подтверждает полем
`total-job-count: 18`; параметры `limit`, `category`, `search` он игнорирует — с ними те же
18 бит в бит. Полнее бесплатно нет ничего: RSS `https://remotive.com/remote-jobs/feed` даёт
20 записей (те же плюс две, которые API прячет 24-часовой задержкой), HTML-страница
`https://remotive.com/remote-jobs` — те же 20, и `?page=2` отдаёт их же. Куда делось
остальное, площадка говорит сама: на странице висит «Unlock All Jobs — 10 653 added this
week», а в юридической врезке ответа API — «We offer a private, paid-for API … (starting
budget is $5k/mo)». Держать её в списке дёшево (один запрос в прогон), но ждать от неё
вакансий не нужно.

**RemoteOK: лента большая, отклик закрыт.** В ленте 99 вакансий, но кнопка Apply работает
только у тех, что работодатель разместил сам (`original: true`) — таких 3 из 99 (три недели
назад было 2 из 100). Остальные RemoteOK собрал с чужих сайтов и продаёт доступ к ссылке за
$14.95/мес; бесплатного выхода с этого экрана нет. Поэтому поиск отбрасывает их **до**
ИИ-скоринга — иначе мы платили бы модели за вакансии, подать на которые нельзя. Ленту, в
отличие от Remotive, расширить можно: параметр `tags` площадка честно обрабатывает, и восемь
запросов по разным тегам дают 566 разных вакансий вместо 99 — вот только открытых среди них
всё равно 7 (1.2%). Узкое место — подписка, а не размер ленты.

> [!IMPORTANT]
> **Wellfound не ищет и не откликается без своего Chrome.** Единственный вход на площадку —
> окно, оставшееся открытым после `make login_wellfound`: Cloudflare привязывает пропуск к
> тому браузеру, который прошёл проверку, поэтому свой (headless или нет) не годится. Если
> окно закрыто, площадка отдаёт ноль — и поиск, и отклик. Раньше понять это можно было
> только по сырому `connect ECONNREFUSED 127.0.0.1:9222` в консоли; теперь про это говорят
> **до** начала работы:
> ```
> ⚠️  Wellfound: Chrome с отладочным портом не отвечает — площадка отдаст ноль
...
> ```
> `make search` — до первого запроса; `make run` — если в очереди есть лид Wellfound; воркер
> — при старте, а на каждом своём поиске ещё и **в Telegram**: он ищет трижды в день неделями,
> и человека у ноута в этот момент нет. Молчит, когда порт отвечает, когда площадка стоит на
> паузе и когда её в этом прогоне нет вовсе (`make search_hh`) — там порт даже не дёргается.

**Сбой сети больше не выглядит как «ничего не нашлось».** У Remotive и RemoteOK поиск раньше
глушил любое исключение и возвращал пустой список: 429 и 503 от площадки читались ровно как
обычный день без совпадений. Теперь ошибка долетает до `run_search`, и площадка получает в
отчёте «ошибка» вместо «пусто» — как у всех остальных.
...
| `connect ECONNREFUSED 127.0.0.1:9222`, Wellfound даёт ноль | Закрыто окно Chrome от `make login_wellfound` — другого входа на площадку нет. Подними и оставь открытым. Прогон и поиск теперь предупреждают об этом сами при старте. |
| Remotive и RemoteOK каждый раз «пусто» | Так и есть: у Remotive публичная лента — 18–20 вакансий всего, у RemoteOK отклик открыт на 3 из 99. Это потолок площадок, а не поломка — см. «Сколько какая площадка реально отдаёт». |
