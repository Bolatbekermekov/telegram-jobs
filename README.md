...
| `GEMINI_MODEL` / `GEMINI_MODEL_CHEAP` | Модели Gemini: пишущая `gemini-3.5-flash`, массовая `gemini-3.5-flash-lite` |
...
У Gemini поэтому единственный случай, когда тиры реально разные:

- **пишущая модель — `gemini-3.5-flash`.** `flash-lite` в трёх письмах из трёх
  вставил русскую строку («Мой стек: …») в английское письмо, `flash` — ни разу.
  Письмо читает человек, смешанный язык в отклике HR это брак. Платим 8–9 с
  вместо 1.5, но вызов один на лид, а между лидами и так стоит пауза.
- **массовая модель — `gemini-3.5-flash-lite`.** Здесь решает не слог, а лимит:
  15 запросов в минуту против ~8 у `flash`.

Модели `gemini-2.5-*` новым ключам отвечают 404 — брать 3.x.
