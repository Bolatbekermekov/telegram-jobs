...
`s/e/k/q`. Дневной лимит (`DAILY_SEND_LIMIT`) по-прежнему действует. Чтобы вернуть
подтверждение, поставь `AUTO_SEND=false` (значение по умолчанию).
...
> оставляет такой лид со статусом `new` для ручной отправки.
