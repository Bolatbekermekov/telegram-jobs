...
> На всякий случай авто-режим **пропускает** сообщения, где остался `[плейсхолдер]`, и
> оставляет такой лид со статусом `new` для ручной отправки. Так же он **не отправляет**
> лид из Threads, у которого контакт определила модель, а не правила: подтверждение — это
> и есть проверка того, как модель прочитала тред, поэтому такой лид тоже остаётся `new`.
