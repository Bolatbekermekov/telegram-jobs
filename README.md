...
| Переменная | Что это |
|---|---|
| `INTAKE_LLM_PROVIDER` / `SENDER_LLM_PROVIDER` | Кто отвечает на вызовы модели: `openai` (по умолчанию) или `nvidia`. См. «Смена LLM-провайдера» ниже |
...
у него OpenAI-совместимый endpoint, поэтому работает тот же SDK: меняются только
ключ, адрес и имена моделей.

Переключатели раздельные, по одному на половину:

```bash
SENDER_LLM_PROVIDER=nvidia   # рассылка и поиск на ноуте
INTAKE_LLM_PROVIDER=openai   # бот-приёмник в облаке
```
...
Если провайдер выставлен в `nvidia`, а `NVIDIA_API_KEY` пуст, приложение падает
на старте с явным сообщением, а не уходит молча в OpenAI: ключи не
взаимозаменяемы, `nvapi-…` в `api.openai.com` даёт 401.
...
4. В **Environment Variables** добавь: `OPENAI_API_KEY`, `OPENAI_MODEL_CHEAP`, `TELEGRAM_BOT_TOKEN`,
   ...
   Захочешь перевести облако на NVIDIA — добавь `INTAKE_LLM_PROVIDER=nvidia` и
   `NVIDIA_API_KEY`, но сначала прочитай «Смена LLM-провайдера» про лимит в 10 секунд.
