...
Sender поддерживает шесть каналов для отправки откликов:
...
> теряется и не уходит в `skipped`, но и сам не отправляется. Основной путь (найти контакт
> в треде) работает и сессии Threads не требует, так что `make login_threads` можно вообще
> не делать, пока эта дырка не закрыта.
